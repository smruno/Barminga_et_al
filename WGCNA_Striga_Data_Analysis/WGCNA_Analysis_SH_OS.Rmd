...
DGES <- read.csv("Rice_Counts_SH.csv")
List_SOM3 <- read.csv("List_SH_OS_DGES.csv")
DGES_Selected = DGES [which(DGES$gene %in% List_SOM3$gene), ]
write.csv(as.data.frame(DGES_Selected),file = "DGES_OS_SH_counts.csv")




data_dir <- file.path("data")
data_file <- file.path(data_dir, "DGES_OS_SH_counts.csv")
metadata_file <- file.path(data_dir, "Col_Info_2.csv")
...
counts.sorghum <- subset(counts.rice, select = names) ## reorder counts columns based on the experimental setup 
dim(counts.rice)
dim(metadata)
...
keep <- rowSums(thresh) >= 10 ## keep genes that have at least 30 TRUES in each row of thresh
...
TOM = TOMsimilarityFromExpr(normalized_counts, power = 14)
dissTOM = 1-TOM 
# Plot gene tree
...
write.csv(as.data.frame(hubs),file = "HUB_TOP_genes_DGE_SH_OS.csv")
```
...
write.csv(as.data.frame(Hubs_Selected),file = "Annotated_Hubs_May_19_power_12.csv")
